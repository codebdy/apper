...
import { useTheme, usePrefix, TextWidget, IconWidget } from '../../../react'
...
export const MonacoInput: React.FC<MonacoInputProps> & {
  ...
  ...props
    ...
    const computedLanguage = useRef<string>((language || defaultLanguage) as any)
    ...
    const onMountHandler = (
      ...
      if (currentValue) {
        format(computedLanguage.current, currentValue)
          .then((content) => {
            editor.setValue(content as any)
            setLoaded(true)
          })
          .catch(() => {
            setLoaded(true)
    ...
    const submit = () => {
      clearTimeout(submitRef.current as any)
    ...
    const validate = () => {
      if (realLanguage.current === 'typescript') {
        clearTimeout(validateRef.current as any)
        validateRef.current = setTimeout(() => {
          try {
            ...
            declarationRef.current = editorRef.current?.deltaDecorations(
              ...
              [
                {
                  range: (monacoRef.current ? new monacoRef.current.Range(1, 1, 1, 1) : undefined) as any ,
                  options: {},
                },
              ]
            ) as any
            ...
            declarationRef.current = editorRef.current?.deltaDecorations(
              ...
              [
                {
                  range: monacoRef.current ? new monacoRef.current.Range(
                    e.loc.line,
                    e.loc.column,
                    e.loc.line,
                    e.loc.column
    ...
    computedLanguage.current = (language || defaultLanguage) as any
